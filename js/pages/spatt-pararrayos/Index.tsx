...
import type { SpattPararrayoSpreadsheetSummary } from '@/types/spatt-pararrayos';

interface PageProps {
    spreadsheets: SpattPararrayoSpreadsheetSummary[];
...
export default function Index() {
    ...
    const handleCreate = (e: React.FormEvent) => {
        e.preventDefault();
        setProcessing(true);
        router.post(spattPararrayosRoutes.store.url(), { name, project_name: projectName }, {
    ...
    const handleJoin = (e: React.FormEvent) => {
        e.preventDefault();
        setProcessing(true);
        router.post(spattPararrayosRoutes.join.url(), { code: code.toUpperCase() }, {
            onFinish: () => { setProcessing(false); setShowJoin(false); setCode(''); },
        });
    };

    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Cálculo SPAT y Pararrayos', href: spattPararrayosRoutes.index.url() },
    ];
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            <div className="w-full rounded-lg bg-white px-12 py-6 shadow dark:bg-gray-900">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div>
                        <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100">Hojas SPAT y Pararrayos</h1>
                ...
                {spreadsheets.length === 0 ? (
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                        {spreadsheets.map((sheet) => (
                            <div key={sheet.id} className="group cursor-pointer rounded-lg border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md dark:border-gray-700 dark:bg-gray-800" onClick={() => router.get(spattPararrayosRoutes.show.url(sheet.id))}>
                                <div className="p-4">
                                        ...
                                        <button
                                            onClick={(e) => {
                                                ...
                                                if (confirm('¿Eliminar esta hoja?')) {
                                                    router.delete(spattPararrayosRoutes.destroy.url(sheet.id));
                                                }
                            ...
                            <form onSubmit={handleCreate} className="space-y-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="Ej: SPAT - Colegio XYZ"
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="Ej: Proyecto Edificio A"
                                        className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 dark:placeholder:text-gray-500"
                                    />
                                </div>
